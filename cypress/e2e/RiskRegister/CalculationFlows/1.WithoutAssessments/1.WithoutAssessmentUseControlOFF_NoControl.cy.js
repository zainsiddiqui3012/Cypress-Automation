...
describe(
  "Calculate All Risks in Risk Register When Assessment is OFF, Manage Detail Control is OFF, Use Control Strength is OFF",
  {
    tags: [
      "@smoke",
      "@without-assessment",
  ...
  () => {
    ...
    before(() => {
      ...
      riskRegister_PO.toggleControlStrengthSetting(false);
        ...
        cy.readFile(riskRegisterData).then((data) => {
          ...
          likelihoodArray.forEach((likelihood) => {
            impactArray.forEach((impact) => {
              ...
              riskRegister_PO.selectLikelihoodValue(likelihood.label);
              riskRegister_PO.selectImpactValue(impact.label);
