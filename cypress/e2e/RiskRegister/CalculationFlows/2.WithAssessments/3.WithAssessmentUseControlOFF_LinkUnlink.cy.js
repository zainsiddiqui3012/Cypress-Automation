...
describe(
  "Calculate All Risks With Assessments Within Risk Register When Assessment is ON, Manage Detail Control is OFF, Use Control Strength is OFF",
  {
    tags: [
      "@smoke",
      "@with-assessment",
  ...
  () => {
    ...
    context("Test with Linked Control", () => {
      ...
      it("When Without Control Strength Control is Linked it will not effect CR and RR of Risk Definition", () => {
        cy.readFile(riskRegisterData).then((data) => {
          const allControlsToAdd = ["Control Definition (Corrective)"];
          // when Assessment is submit/Linked Dom starts with new state
          cy.wait(1500);
