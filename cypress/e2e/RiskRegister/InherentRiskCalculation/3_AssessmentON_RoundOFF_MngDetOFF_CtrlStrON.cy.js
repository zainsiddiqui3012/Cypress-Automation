...
describe(
  ...
  () => {
    ...
    context("Disable Manage Detail Control, Round decimal, Enable Assessment, Control Strength", () => {
      ...
      before(() => {
        ...
        cy.configureSettings({ assessment: true, round: false, manageDetailControl: false, controlStrength: true });
      ...
      it("Validate the value in inherent risk probability assessment option",
        {
          tags: [
            "@pd45632",
            "@smoke"
      ...
      it("When Assessment is ON, Round Decimal Value Flag off",
        {
          tags: [
            "@pd45646",
            "@smoke"
        ...
        () => {
          ...
          .then((result) => {
            expect(result.inherentLikelihood).to.equal(data.expectedInherentLikelihoodRoundOff);
          });
          ...
          inherentRiskCalculation.getRiskFromDB(data.customerId, data.riskId)
          .then((result) => {
            expect(result.inherentImpact).to.equal(data.expectedInherentImpactRoundOff);
          });
          ...
          inherentRiskCalculation.getRiskFromDB(data.customerId, data.riskId)
          .then((result) => {
            expect(result.inherentRiskRating).to.equal(data.expectedInherentRiskRoundOff);
          });
