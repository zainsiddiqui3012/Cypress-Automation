...
describe.skip("Risk Category Screen Automation - Risk Calculation", () => {
  ...
  riskCategory.forEach((test) => {
    it("Add Category Successfully for Calculation", () => {
      //Login Details
      cy.log(
        Cypress.env("username3"),
        Cypress.env("password3"),
        Cypress.env("key3")
      );
...
describe.skip("Risk Defintion Screen Automation - Risk Calculation", () => {
  const loginDetails_PO = new LoginDetails_PO();
  const predictMenu_PO = new PredictMenu_PO();
...
describe.skip("Mark Applicable from Risk Register Three Ellipsis Options - Risk Calculation", () => {
  const loginDetails_PO = new LoginDetails_PO();
  const predictMenu_PO = new PredictMenu_PO();
...
describe.skip("Inherent Risk and Residual Risk Calculation via Risk Analysis Dimensions  (Feature : Disable -Use Assessments to Calculate Inherent Risk and Feature : Enabled - Manage Detailed Controls))", () => {
  const loginDetails_PO = new LoginDetails_PO();
  const predictMenu_PO = new PredictMenu_PO();
  const riskRegister_PO = new RiskRegister_PO();
...
//             RiskCalculation_PO.riskDimensionsTab1(test.Likelihood_Tab1_Label, test.Likelihood_Tab1_Value, test.Likelihood_Tab1_LabelResidual, test.Likelihood_Tab1_ValueResidual);
//             RiskCalculation_PO.riskDimensionsTab2(test.Impact_Tab2_Label, test.Impact_Tab2_Value, test.Impact_Tab2_LabelResidual, test.Impact_Tab2_ValueResidual);
...
//             const IR = parseFloat(test.Likelihood_Tab1_Value * test.Impact_Tab2_Value).toFixed(4)
...
//             cy.wait(10000);
//             cy.iframe('#iframe-risk-register-control-modal').find('div.form:nth-child(1) > div:nth-child(1) > div:nth-child(2) > table:nth-child(1) > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(1)').contains(IR);
