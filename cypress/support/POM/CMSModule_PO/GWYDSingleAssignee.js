...
const filename = 'cypress/fixtures/CMSIssueExternalWebforms/TicketIDSingleGWYD.txt'
const filename2 = 'cypress/fixtures/CMSIssueExternalWebforms/TicketIdSubtaskSingleGWYD.txt'

class GWYDSingleAssignee {
    setCustomerProfile(issueOwnerType, issueOwnerTypeName, customerProfileUpdatemsg) {
        cy.get("[name='issueProcessOwnerType'][value='Single']").click({force:true});
        cy.wait(3000);
        cy.get('#s2id_issueProcessOwnerId > .select2-choice').click();
        cy.wait(4000);
        cy.get('#s2id_autogen8_search').type(issueOwnerTypeName).wait(3000).type('{enter}');
        cy.wait(4000);
        cy.get('.m-form__actions > .btn-primary').click({force:true});
        cy.xpath('//*[@id="toast-container"]').contains(customerProfileUpdatemsg);
    }
    validateMandatoryFields(assigneeError, errorMessageSummary, errorMessageSubName, errorMessageIssueIdentificationDate, errorMessageIssueDescrp, errorMessageResDept, errorMessageOwners) {
        ...
        cy.switchIframe('#mytarget').find('#issue-create > div.form-body > div.aui-message.aui-message-error > p:nth-child(2)').contains(errorMessageSubName,);
        cy.switchIframe('#mytarget').find('#issue-create > div.form-body > div.aui-message.aui-message-error > p:nth-child(3)').contains(errorMessageIssueIdentificationDate,);
        cy.switchIframe('#mytarget').find('#issue-create > div.form-body > div.aui-message.aui-message-error > p:nth-child(4)').contains(errorMessageIssueDescrp);
        cy.switchIframe('#mytarget').find('#issue-create > div.form-body > div.aui-message.aui-message-error > p:nth-child(5)').contains(errorMessageResDept);
        cy.switchIframe('#mytarget').find('#issue-create > div.form-body > div.aui-message.aui-message-error > p:nth-child(6)').contains(errorMessageOwners);

    }

    fillIssueExternalForm(summaryForm, responsibleDept, issueType, issueDescription, agencyEntities, severityForm, submitterName, assignee_owner, issueSource, successMessage) {
        ...
        cy.switchIframe('#mytarget').find('#assign-to-me-trigger').click();
        cy.wait(3500);
        cy.switchIframe('#mytarget').find('#issue-create-submit')
        .scrollIntoView()
        .should('be.visible')
        .click();
        //adding static wait for detachment of IFrame DOM its known cypress issue, also it will wait for the created ticket
        cy.wait(60000);
    ...
    openTicket(assigneeName) {
        // cy.switchIframe('#mytarget').find('#main > div > div.issue-search-header > form > div.aui-group > div.aui-item.search-wrap > div.search-container > div.search-options-container > span').click();
        // cy.wait(10000);
        cy.switchIframe('#mytarget').find('#main > div > div.issue-search-header > form > div.aui-group > div.aui-item.search-wrap > div.search-container > div.search-field-container > div > div.search-criteria > ul > li:nth-child(4) > div').click();
        cy.wait(10000);
        cy.switchIframe('#mytarget').find('#assignee-input').type(assigneeName).wait(1000).type('{enter}');
        cy.wait(10000);
    ...
    issueFormDataValidation(summaryForm, submitterName, responsibleDept, issueType, issueDescription, agencyEntities, severityForm, assignee_owner, issueSource, issueFormType, issueStatus) {
    ...
    issueFormSingleWorkflowProcess(acceptanceComment, statusAfterAccept, Subtasksummary, Description, prioritySubtask, orgHeirarchy, subjectArea) {
        ...
        cy.wait(5000);
        cy.switchIframe('#mytarget').find('#s2id_select_customfield_18700').type(orgHeirarchy, '{enter}');
    ...
    workflowSingleAssigneeProcessAfterSubtask(resolveComment, issueResolveStatus, acceptedComment, issueAcceptedStatus) {
        cy.wait(20000)
        cy.readFile(filename).then(text => {
            cy.switchIframe('#mytarget').find('#quickSearchInput').type(text + '{enter}')
        });
    ...
    workflowSingleAssigneeProcessReturnToOwner(resolveComment, issueResolveStatus, returntoOwnerButton, returntoownerComment, returnToOwnerStatus, resolveAfterRTO, acceptedComment, issueAcceptedStatus) {
        cy.readFile(filename).then(text => {
            cy.switchIframe('#mytarget').find('#quickSearchInput').type(text + '{enter}')
        });
        cy.wait(30000);
    ...
    workflowSingleAssigneeProcessReopen(acceptanceComment, statusAfterAccept, Subtasksummary, Description, prioritySubtask, orgHeirarchy, subjectArea) {
        const dueDate = dayjs().day(5).format("DD/MMM/YYYY");
        cy.log(dueDate);
        ...
        cy.wait(20000);
        cy.switchIframe('#mytarget').find("#issuetype-field",{timeout:15000}).clear().type("Action Plan",{delay:200}).wait(400).type('{enter}');
        cy.wait(15000);
        cy.switchIframe('#mytarget').find("[name='summary']",{timeout:5000}).type(Subtasksummary,{delay:150});
        cy.wait(2000);
        cy.get("#mytarget").then(function ($iframe1) {
            const iframe2 = $iframe1.contents().find('iframe')
            cy.wrap(iframe2).as('iframe2ref')
        ...
        cy.wait(30000);
        //Subtask click
        cy.switchIframe('#mytarget').find("td[class='stsummary'] a[class='issue-link']").click({ force: true });
    ...
    subTaskWorkflowProcessAfterReject(subtasksubmitButton, statusSubtask, acceptComment) {
        cy.readFile(filename2).then(text => {
            cy.switchIframe('#mytarget').find('#quickSearchInput').type(text + '{enter}')
        });
        cy.wait(30000);
    ...
    rejectSubTask(statusSubtaskreject, submittedStatus, rejectSubtaskButton, rejectSubtaskStatus) {
        ...
        cy.switchIframe('#mytarget').find('#issue-workflow-transition-submit').click();
        cy.wait(40000);
        cy.switchIframe('#mytarget').find('#status-val',{timeout:15000}).contains(rejectSubtaskStatus);
        cy.wait(3000);
...
export default GWYDSingleAssignee;