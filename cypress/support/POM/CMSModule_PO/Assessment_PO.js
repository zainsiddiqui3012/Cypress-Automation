...
const filename = 'cypress/fixtures/CMSModule/TicketId.txt'
const filepath = 'cypress/attachment/testing.txt';

class Assessment_PO{

    
    fillForm(summary,assessmentTemplate,assessmentCategory,ticketIdnew) {
        cy.switchIframe('#mytarget').find("[name='summary']").type(summary);
        cy.switchIframe('#mytarget').find('#assign-to-me-trigger').click({force:true});
        cy.switchIframe('#mytarget').find('#customfield_16704-input').type(assessmentTemplate).wait(7000).type('{enter}');
        // cy.switchIframe('#mytarget').find('#customfield_16704-input').type(assessmentTemplate);
        // cy.wait(7000)
    ...
    validateDataEdit(description,singleAssigneename, Recurrence){
        cy.get('#mytarget').then($iframe => {
            const body = $iframe.contents().find('body')
            cy.wrap(body).as('iframe')
        })
        // //description
        // cy.get('@iframe').find('input#summary.text.long-field').contains(summary);
        // cy.wait(2000)
        cy.get('@iframe').find('#description-val > div > p').contains(description);
        cy.wait(2000)
        cy.get('@iframe').find('span#assignee-val').contains(singleAssigneename);
        cy.wait(2000)
    ...
    workflowProcessTemplate1(statusInprogress,q1Response,q2Response,templateDraft_status,templateSubmit_status){
        ...
        cy.wait(30000);
        cy.switchIframe('#mytarget').find('#action_id_721 > span').should('be.visible').click();
        cy.wait(10000)
        //verify status as in progress
        cy.switchIframe('#mytarget').find('span#status-val').contains(statusInprogress);
        cy.wait(2000)
        //open template
        // cy.switchIframe('#mytarget').find('#descriptiont-val > div >table >tbody >tr >td > a[target="_blank"]').invoke('removeAttr', 'target').click();
    ...
    reviewAssessment(reviewMessage,reviewComment,reviewStatus,q1Response,q2Response){
        cy.switchIframe('#mytarget').find('#action_id_741 > span').should('be.visible').click();
        cy.wait(10000)
        ...
        cy.wait(20000)
        // //expand all
        cy.switchIframe('#mytarget').find('.btn.btn-primary.surResultSectionTglr').should('be.visible').click();
        cy.wait(20000)
        //cy.switchIframe('#mytarget').find('table[id="tblQuestion{283298}Answers"] th:nth-child(2)').should('contain.text', q1Response);
        cy.switchIframe('#mytarget').find('#section_detail_102390_1_1 > div:nth-child(1)').should('contain.text', q1Response);
        cy.wait(10000)
       // cy.switchIframe('#mytarget').find('table[id="tblQuestion{283299}Answers"] th:nth-child(2)').should('contain.text', q2Response);
        cy.switchIframe('#mytarget').find('#section_detail_102390_1_1 > div:nth-child(2)').should('contain.text', q2Response);
        cy.wait(10000)
    ...
    workflowProcessAfterReopen(statusInprogress,templateSubmit_status){
        //start assessment
        //verify status as in progress
    ...
    workflowProcessTemplate3(statusInprogress,q1Response,q2Response,q2Response2,q3Response,q4Response,q5Response,q6Response,q7Response,q8Response,templateSubmit_status)
    {   cy.wait(20000);
        cy.readFile(filename).then(text => {
       cy.switchIframe('#mytarget').find('#quickSearchInput').type(text +'{enter}')        
   });
...
fillFormUnassignee(Unassignee,errorMessage){
    // cy.switchIframe('#mytarget').find("[name='summary']").type(summary);
    // cy.wait(1000)
        cy.switchIframe('#mytarget').find('input#assignee-field').type(Unassignee);
        cy.wait(50000)
        cy.switchIframe('#mytarget').find('#issue-create-submit').click({force:true});
        cy.wait(1000)
        cy.switchIframe('#mytarget').find('div#singleassigneeErrorDiv').contains(errorMessage);    
}
}
export default Assessment_PO;