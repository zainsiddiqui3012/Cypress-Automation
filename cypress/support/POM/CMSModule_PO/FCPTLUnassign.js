...
class FCPTLUnAssignee
{
    ...
    validateMandatoryFields(errorMessageIssueSource,errorMessageOwner,errorMessageIssueDescrp,errorMessageIssueType,errorMessageIssueIdenDate,errorMessageSubName,errorMessageSummary){
        ...
        cy.get('div:nth-of-type(3) > .toast-message').contains(errorMessageIssueDescrp);
        cy.get('div:nth-of-type(4) > .toast-message').contains(errorMessageIssueType);
        cy.get('div:nth-of-type(5) > .toast-message').contains(errorMessageIssueIdenDate);
        cy.get('div:nth-of-type(6) > .toast-message').contains(errorMessageSubName);
        cy.get('div:nth-of-type(7) > .toast-message').contains(errorMessageSummary);
    }
    fillIssueExternalForm(summaryForm,submitterName,responsibleDept,issueType,issueDescription,potentialLoss,actualLoss,rootCause,owner,issueSource,errorMessage)
    {
...
export default FCPTLUnAssignee;