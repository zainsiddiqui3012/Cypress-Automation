...
class IssueWorkflowGroupAssignee_PO {

    workflowGrpAssigneeProcess(acceptanceComment,statusAfterAccept,Subtasksummary,Description,prioritySubtask,site,subjectArea)
    {
        ...
        cy.wait(20000);
        cy.switchIframe('#mytarget').find('#issuetype-field',{timeout:10000}).clear().type('Action Plan',{delay:200}).wait(700).type('{enter}');
        cy.wait(6000);
        cy.switchIframe('#mytarget').find("[name='summary']").wait(5000).type(Subtasksummary);
        cy.wait(5000);
        cy.get("#mytarget").then(function($iframe1){
    ...
    workflowGrpAssigneeProcessAfterSubtask(resolveComment,issueResolveStatus,acceptedComment,issueAcceptedStatus)
    {
        cy.readFile(filename).then(text => {
            cy.switchIframe('#mytarget').find('#quickSearchInput').type(text +'{enter}')        
        });
    ...
    workflowGrpAssigneeProcessReturnToOwner(resolveComment,issueResolveStatus,returntoOwnerButton,returntoownerComment,returnToOwnerStatus,resolveAfterRTO,acceptedComment,issueAcceptedStatus)
    {
        cy.readFile(filename).then(text => {
            cy.switchIframe('#mytarget').find('#quickSearchInput').type(text +'{enter}')        
        });
    ...
    workflowGrpAssigneeProcessReopen(acceptanceComment,statusAfterAccept,Subtasksummary,Description,prioritySubtask,site,subjectArea)
    {
        const dueDate = dayjs().day(5).format("DD/MMM/YYYY");
        cy.log(dueDate);
        ...
        cy.wait(30000);
        //Subtask click
        cy.switchIframe('#mytarget').find("tr:nth-of-type(2) > .stsummary > .issue-link").click();
        cy.wait(20000);
        cy.switchIframe('#mytarget').find('a#key-val').invoke('text').as('ticketIdsubtask');
        cy.wait(1000)
        cy.get('@ticketIdsubtask').then((ticketIdsubtask) => {
...
export default IssueWorkflowGroupAssignee_PO;