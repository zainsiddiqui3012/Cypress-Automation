...
class IssueManagementUnassignee_PO
{
    setCustomerProfile(issueOwnerType,customerProfileUpdatemsg)
    {
        cy.get('.form-group.m-form__group.row.pb-3:nth-child(32) > .col-md-6 > .m-radio-inline').contains(issueOwnerType).click();
        cy.wait(4000);
        cy.get('.m-form__actions > .btn-primary').click();
        cy.get('div.toast-message').contains(customerProfileUpdatemsg);
    }
    validateMandatoryFields(errorMessageSummary,errorMessageIssueType,errorMessageSubName,errorMessageIssueIdenDate,errorMessageIssueDescrp,errorMessageIssueSource,errorMessageAgency,errorMessageOwner){
    ...
    fillIssueForm(summary,IssueSource,Agency_Entities,typeOfIssue,issueDescription,audit_exam,issueOwner,rootCause,rootCauseDescription,submitterName, repeatFinding, priority, projectNumber, subjectArea, severity){
    ...
    {   cy.readFile(filename).then(text => {
        ...
        cy.wait(40000);
        cy.switchIframe('#mytarget').find('a#edit-issue').click({force:true});
        cy.wait(40000);
...
export default IssueManagementUnassignee_PO;