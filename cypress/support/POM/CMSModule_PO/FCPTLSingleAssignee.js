...
const filename = 'cypress/fixtures/CMSIssueExternalWebforms/TicketIDSingle.txt'
const filename2 = 'cypress/fixtures/CMSIssueExternalWebforms/TicketIdSubtaskSingle.txt'

class FCPTLSingleAssignee {
    setCustomerProfile(issueOwnerType, issueOwnerTypeName, customerProfileUpdatemsg) {
        ...
        cy.wait(4000);
        cy.get('#s2id_autogen5_search').type(issueOwnerTypeName).wait(3000).type('{enter}');
        cy.wait(4000);
    ...
    issueFormSingleWorkflowProcess(acceptanceComment, statusAfterAccept, Subtasksummary, Description, prioritySubtask, orgHeirarchy, subjectArea) {
        ...
        cy.wait(12000);
        cy.switchIframe('#mytarget').find("#summary").type(Subtasksummary, {force : true});
        cy.wait(2000);
        cy.get("#mytarget").then(function ($iframe1) {
    ...
    subTaskWorkflowProcess(subtasksubmitButton, statusSubtask, actionPlanStatus, submittedStatus, acceptComment, subtaskcloseStatus) {
        ...
        cy.wait(30000);
        cy.switchIframe('#mytarget').find('#action_id_11').contains(subtasksubmitButton).click();
        cy.wait(30000);
...
export default FCPTLSingleAssignee;